...
- **Docker-Deployment** — `docker-compose.yml` mit Traefik-Labels für `your-domain.example.com`; separates `Dockerfile.server` für den Node.js-Backend-Container; `nginx.conf` mit `/api/*`-Proxy zum Backend
